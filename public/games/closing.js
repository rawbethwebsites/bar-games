function createClosingGame(app) {
  ...
  async function load() { const res = await fetch('games/closing.json'); bank = await res.json(); }
  function setHint() {
  ...
  function renderPick() {
    stage.innerHTML = `
      <div class="stage-wrap">
        <div class="stage-q"><div class="lab">Round ${round} / ${MAXR} — closing argument</div>
  ...
  function pick(side, idx) {
    ...
    document.getElementById('stat-' + side).textContent = 'Locked in!';
  ...
  function openVote() {
    ...
    overlay.innerHTML = `
      <h3>Closing arguments — which persuaded the room?</h3>
