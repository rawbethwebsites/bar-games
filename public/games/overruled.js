function createOverruledGame(app) {
  const MAX = 10, QTIME = 9;
  const stage = document.getElementById('game-stage');
  let bank = [], deck = [], round = 0, cur = null;
  let ans = { red: null, blue: null }, timer = null, resolving = false;

  async function load() { const res = await fetch('games/overruled.json'); bank = await res.json(); }
  function shuffle(a) { a = a.slice(); for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; } return a; }
  function setHint() {
    const h = document.querySelector('.controls-hint');
    if (h) h.innerHTML = '<span><b>LEFT:</b> A = SUSTAINED · D = OVERRULED</span><span><b>RIGHT:</b> J = SUSTAINED · L = OVERRULED</span>';
  }
  ...
  function render() {
    stage.innerHTML = `
      <div class="stage-wrap">
        <div class="stage-q">
          <div class="lab">Motion ${round} / ${MAX} — on the bench</div>
          <div class="qtext">${cur.motion}</div>
        ...
        <div class="split-cols">
          <div class="split-col red"><div class="tag">🔴 Prosecution</div>
            <div class="choices">
              <button class="choice-btn choice-real" data-side="red" data-ans="true">SUSTAINED</button>
              <button class="choice-btn choice-fake" data-side="red" data-ans="false">OVERRULED</button>
            </div>
            <div class="col-status" id="stat-red"></div></div>
          <div class="split-col blue"><div class="tag">🔵 Defence</div>
            <div class="choices">
              <button class="choice-btn choice-real" data-side="blue" data-ans="true">SUSTAINED</button>
              <button class="choice-btn choice-fake" data-side="blue" data-ans="false">OVERRULED</button>
            </div>
  ...
  function resolve() {
    ...
    let rp = 0, bp = 0;
    if (ans.red === cur.sustained) rp = 1;
    if (ans.blue === cur.sustained) bp = 1;
