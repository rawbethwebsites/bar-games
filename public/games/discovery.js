function createDiscoveryGame(app) {
  const MAX = 10, QTIME = 9;
  const stage = document.getElementById('game-stage');
  let bank = [], deck = [], round = 0, cur = null;
  let ans = { red: null, blue: null }, timer = null, resolving = false;

  async function load() { const res = await fetch('games/discovery.json'); bank = await res.json(); }
  function shuffle(a) { a = a.slice(); for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; } return a; }
  function setHint() {
    const h = document.querySelector('.controls-hint');
    if (h) h.innerHTML = '<span><b>LEFT:</b> A = COMPLY · D = OBJECT</span><span><b>RIGHT:</b> J = COMPLY · L = OBJECT</span>';
  }
  ...
  function render() {
    stage.innerHTML = `
      <div class="stage-wrap">
        <div class="stage-q">
          <div class="lab">Request ${round} / ${MAX} — discovery dispute</div>
          <div class="qtext">${cur.request}</div>
        ...
        <div class="split-cols">
          <div class="split-col red"><div class="tag">🔴 Prosecution</div>
            <div class="choices">
              <button class="choice-btn choice-real" data-side="red" data-ans="true">COMPLY</button>
              <button class="choice-btn choice-fake" data-side="red" data-ans="false">OBJECT</button>
            </div>
            <div class="col-status" id="stat-red"></div></div>
          <div class="split-col blue"><div class="tag">🔵 Defence</div>
            <div class="choices">
              <button class="choice-btn choice-real" data-side="blue" data-ans="true">COMPLY</button>
              <button class="choice-btn choice-fake" data-side="blue" data-ans="false">OBJECT</button>
            </div>
  ...
  function pick(side, answer) {
    ...
    const correct = answer === (cur.bestChoice === 'comply');
  ...
  function resolve() {
    ...
    const target = cur.bestChoice === 'comply';
    let rp = 0, bp = 0;
  ...
  // sustainoverrule scheme: 'a' = left/comply, 'b' = right/object
